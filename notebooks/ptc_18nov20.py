    ...
    **kwargs: `dict`
    ...
    def runDataRef(self, dataRefList):
        ...
        if self.config.ptcFitType in ["FULLCOVARIANCE", ]:
            ...
            for ampName in datasetPtc.ampNames:
                ...
                datasetPtc.badAmps = newDatasetPtc.badAmps
    ...
    def getOutputPtcDataCovAstier(self, dataset, covFits, covFitsNoB):
        ...
        for i, amp in enumerate(dataset.ampNames):
            ...
            if (amp in covFits and (covFits[amp].covParams is not None) and
                ...
                dataset.expIdMask[amp] = np.repeat(False, lenInputTimes)
    ...
    def fitPtc(self, dataset, ptcFitType):
        ...
        for i, ampName in enumerate(dataset.ampNames):
            ...
            # First eliminate points beyond which the variance decreases
            mask = np.array([True for m in meanVecOriginal])
            print("diff", np.diff(varVecOriginal))
            ...
            if len(pivot) > 0:
                ...
                pivot = [p for p in pivot if varVecOriginal[p] > 10000]
                if len(pivot) > 0:
                    pivot = np.min(pivot)
                    print("pivot", pivot)
    ...
    def fillBadAmp(self, dataset, ptcFitType, ampName):
        """Fill the dataset with NaN if there are not enough good points
