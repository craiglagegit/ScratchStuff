...
from .utils import (irlsFit, fitLeastSq, fitBootstrap, funcPolynomial, funcAstier)
...
class MeasurePhotonTransferCurveTask(pipeBase.CmdLineTask):
    ...
    **kwargs: `dict`
    ...
    def runDataRef(self, dataRefList):
        ...
        for expTime, (exp1, exp2) in expPairs.items():
            ...
            for ampNumber, amp in enumerate(detector):
                ...
                muDiff, varDiff, covAstier = self.measureMeanVarCov(exp1, exp2, ampName, expId1, region=amp.getBBox(),
                                                                    covAstierRealSpace=doRealSpace)
                if covAstier is not None:
                    nPixels = np.array([x[4] for x in covAstier])
                    if nPixels.any() == 0:
                        print("Found a zero in covAstier npix", ampName, muDiff)
        ...
        if self.config.ptcFitType in ["FULLCOVARIANCE", ]:
            ...
            if ampName not in datasetPtc.badAmps:
                # Need this in case the amp was found to be bad in fitPtc
    ...
    def measureMeanVarCov(self, exposure1, exposure2, ampName, expId1, region=None, covAstierRealSpace=False):
        ...
        if np.sum(w) < 10000: # Too few pixels to get a valid result
            print("Too few pixels", ampName, expId1)
            return np.nan, np.nan, None
    ...
    def fitPtc(self, dataset, ptcFitType):
        ...
        for i, ampName in enumerate(dataset.ampNames):
            ...
            if ptcFitType == 'EXPAPPROXIMATION':
                ptcFunc = funcAstier
                parsIniPtc = [-1e-9, 1.0, 100.]  # a00, gain, noise
                # lowers and uppers obtained from studies by C. Lage (UC Davis, 11/2020).
                bounds = self._boundsForAstier(parsIniPtc, lowers=[-1e-4, 0.5, -1000],
            ...
            parsFit, parsFitErr, reducedChiSqPtc, weightsY = irlsFit(parsIniPtc, meanVecOriginal,
                                                                     varVecOriginal, ptcFunc,
                                                                     weightsY=1./np.sqrt(varVecOriginal))
            sigResids = (varVecOriginal - ptcFunc(parsFit, meanVecOriginal))/np.sqrt(varVecOriginal)
            mask = np.array([True if np.abs(r) < sigmaCutPtcOutliers else False for r in sigResids])
            ...
            # Now eliminate points beyond which the variance decreases
            print("varVecFinal", varVecFinal)
            print("diff", np.diff(varVecFinal))
            pivot = np.where(np.array(np.diff(varVecFinal)) < 0)[0]

            if len(pivot) > 0:
                # For small values, sometimes the variance decreases slightly
                # Only look when var > 10000.  Need a config option for this?
                pivot = [p for p in pivot if varVecFinal[p] > 10000]
                if len(pivot) > 0:
                    pivot = np.min(pivot)
                    fullPivot = int(np.where(varVecOriginal == varVecFinal[pivot])[0])
                    print("fullPivot", fullPivot)
                    mask[fullPivot+1:len(mask)] = False
            ...
            print(f"Discarded {nDroppedTotal} points in total for {ampName}")
            # objects should never shrink
            assert (len(mask) == len(timeVecOriginal) == len(meanVecOriginal) == len(varVecOriginal))

            # Re-fit with only remaining points
            parsFit, parsFitErr, reducedChiSqPtc, weightsY = irlsFit(parsFit, meanVecFinal,
                                                                     varVecFinal, ptcFunc,
                                                                     weightsY=1./np.sqrt(varVecFinal))
            print("Final Reults", ampName)
            print("parsFit",parsFit)
            print("weightsY", weightsY)
            sigResids = (varVecFinal - ptcFunc(parsFit, meanVecFinal))/np.sqrt(varVecFinal)
            print("sigResids", sigResids)
            print("mask", mask)
